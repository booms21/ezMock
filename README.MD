...
| objectFieldType    | string[] | 见默认值 | 对象字段类型列表, 根据类型数据生成对应类型的内容，支持 "string", "number", "boolean" , "object","array","url","image","time","color","base64Image" |
| maxObjectArraySize | number   | 500      | 当 isObjectArray 为 true 成功时返回的对象数组的长度                                                                                                |
